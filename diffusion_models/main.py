...
def main():
    ...
    for i in range(len(classes)):
        ...
        diffusion = dfn.forward_diffusion(dataset, NUM_STEPS, plot=False)
        ...
        try:
            ...
            model = dfn.reverse_tabular_diffusion(dataset, diffusion, NUM_REVERSE_STEPS, plot=False, model=model)
        except:
            model = ConditionalModel(NUM_STEPS, dataset.size(1))
            model = dfn.reverse_tabular_diffusion(dataset, diffusion, NUM_REVERSE_STEPS, plot=False, model=model)
    ...
    for i in range(len(classes)):
        ...
        diffusion_output = utils.get_model_output(model, input_size, ddpm, num_to_gen)
